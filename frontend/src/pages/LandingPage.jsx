...
function LandingPage() {
  ...
  const { loading: authLoading, signInWithGoogle } = useAuth();
  ...
  // When both conditions are satisfied, trigger fade then unmount loading screen
  useEffect(() => {
    if (minTimeElapsed && !authLoading && !loadingDone) {
      setFadeOut(true);
      const timer = setTimeout(() => setLoadingDone(true), 600); // matches CSS animation
      return () => clearTimeout(timer);
    }
  }, [minTimeElapsed, authLoading, loadingDone]);

  // --- Landing page logic (only runs after loading) ---
      ...
      <div className="content-layer">
        <div className="hero-content">
          <div className="text-container">
            <TextType
              as="h1"
              className="title type-title"
              text="OrsusHealth"
              loop={false}
              typingSpeed={128}
              initialDelay={180}
              showCursor
              cursorClassName="type-cursor font-light"
