"""
Python port of frontend/src/lib/clinical_facts.js

Provides clinical facts, suggestions, and citations for each cardiovascular
feature used by the heart-disease prediction model.  Used by the Dr. Bear
LangChain agent as a tool call so it can look up medical reference data
on demand.
"""

CLINICAL_FACTS: dict[str, dict[bool, dict[str, str]]] = {
    "Age": {
        True: {
...
def get_clinical_facts(feature_name: str, is_positive: bool) -> dict[str, str]:
    """Return the clinical fact, suggestion, and citation for a feature.

    Parameters
    ----------
    feature_name : str
        The feature key, e.g. ``"ST_Slope_Flat"`` or ``"Age"``.
    is_positive : bool
        ``True`` when the feature *increases* cardiovascular risk according
        to the ML model's SHAP values; ``False`` otherwise.
    """
    feature_record = CLINICAL_FACTS.get(feature_name)
    if feature_record is None:
        return dict(_FALLBACK)
    return dict(feature_record.get(is_positive, feature_record[True]))
